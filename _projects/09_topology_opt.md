...
Real materials are not pristine. They carry micro-scale damage — micro-cracks, voids, degraded regions — and that damage changes how stiff and how strong the material is locally. This course project investigated **topology optimisation within two-scale damaged structures**, with a practical slant toward recovery and prevention: the structures that most need protecting are the ones that have already been optimised, because an optimised structure has no spare material and is therefore fragile the moment damage appears. The method couples a fine, micro-scale picture of where the material is damaged to the coarse, macro-scale question of where material should be placed, and then lets the optimiser route material around the damaged regions rather than through them. Damage itself enters the model as a local loss of compliance — a degraded region simply carries less load than the intact material around it. The interesting tension is that the two scales talk to each other. Damage at the micro-scale alters the effective properties the macro-scale optimiser sees, which changes the optimal layout, which in turn changes how the structure is loaded and therefore where damage accumulates.
...
I implemented the whole thing by hand in MATLAB — the optimiser and the two-scale coupling both, back before coding agents could write any of it for you. It is a slow way to build something, but it leaves you no illusions about what the code is actually doing. For me this project was about learning to think across scales at once, and to be suspicious of any optimisation whose "optimum" quietly depends on an idealisation of the material — a scepticism that has stuck with me in every optimisation problem since.
